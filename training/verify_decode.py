...
from training.data.discover import download_episodes, list_episodes_by_task, select_episodes
from training.data.episode import align_episode, chunk_actions
from training.data.prepare import require_token
...
def main() -> None:
    ...
    cfg = DataConfig()
    token = require_token(cfg.source_uri)
    by_task = list_episodes_by_task(
        token, cfg.source_uri, path_to_split_and_task=cfg.robot.path_to_split_and_task,
    )
    selected = select_episodes(by_task, [args.task], 1)
    local = download_episodes(token, cfg.source_uri, selected, cfg.cache_dir)
    ...
    for k, frames in list(raw.cameras.items()):
        ...
        raw.cameras[k] = decode_camera_stream(frames, cfg.image_size)
        print(f"           -> decoded {len(raw.cameras[k])} frames")

    aligned = align_episode(raw, cfg)
    t = aligned["state"].shape[0]
    print(f"\naligned to {t} ticks at {cfg.robot.tick_fps} fps ({t / cfg.robot.tick_fps:.1f}s)")
