...
| `NCCL_*` (any NCCL env var, e.g. `NCCL_NVLS_ENABLE`, `NCCL_P2P_DISABLE`) | whatever NCCL's own defaults are | exporting one in your shell before running `train.py`/`prepare_data.py` is NOT enough on its own -- Ray Train workers are spawned by Ray's own actor infrastructure, not plain child processes of this driver, confirmed by a real repro where a shell-exported `NCCL_NVLS_ENABLE=0` never reached the worker where NCCL actually initializes. `training/common/ray_setup.py`'s `build_runtime_env()` forwards any `NCCL_*` var already in your shell's environment into Ray's own `runtime_env.env_vars` explicitly, so exporting it before running still works, it just needs this forwarding to actually reach the workers. Relevant on some shared/virtualized multi-GPU instances where NVLink SHARP multicast isn't fully exposed to the container -- real symptom: `NCCL error ... Failed to bind NVLink SHARP (NVLS) Multicast memory ... CUDA error 401`, fixed by exporting `NCCL_NVLS_ENABLE=0` (and sometimes also `NCCL_P2P_DISABLE=1`) before running |
